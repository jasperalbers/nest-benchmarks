...
n_surr = 200           # Number of surrogates for the bootstrapping method
...
neurons_per_link = 10
for i in range(5): # number links
    for j in range(neurons_per_link): # number of neurons per link
        sts[i*neurons_per_link+j] = neo.SpikeTrain(np.append(sts[i*neurons_per_link+j], 100*pq.ms+binsize*i).magnitude,
                                                   units=sts[i*neurons_per_link+j].units,
                                                   t_start=0,
                                                   t_stop=T) # first repetition of chain
        sts[i*neurons_per_link+j] = neo.SpikeTrain(np.append(sts[i*neurons_per_link+j], 500*pq.ms+binsize*i).magnitude,
                                                   units=sts[i*neurons_per_link+j].units,
                                                   t_start=0,
                                                   t_stop=T) # second repetition of chain
